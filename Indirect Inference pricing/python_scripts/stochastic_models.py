...
def add_microstructure_noise_logprice(price_paths: np.ndarray, eta: float, seed: int = 42) -> np.ndarray:
    """Add microstructure noise on log-prices, ensuring positive observed prices."""
    rng = np.random.default_rng(seed)
    logS = np.log(np.asarray(price_paths, dtype=float))
    eps = rng.normal(loc=0.0, scale=eta, size=logS.shape)
    logP_obs = logS + eps
    return np.exp(logP_obs)
