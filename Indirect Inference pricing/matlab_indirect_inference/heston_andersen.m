function [log_asset_trj, vola_trj] = heston_andersen(mu, k, theta, sigma, rho, X0, nu0, n_steps, dt, w1, w2)
% This function simulates the Heston model by Andersen
% QE (quadratic exponential) methodology
...
log_asset_trj = zeros(n_steps, 2*n); % Allocating the memory for the log-asset trajectory 
vola_trj = zeros(n_steps, 2*n); % allocating the memory for the volatility trajectory

log_asset_trj(1, :) = X0; % fixing the initial value of the log-asset
...
gaussian_noise = sqrt(2) * erfinv(2*w - 1);
...
for j=2:n_steps
    ...
    inv_unif = 1./beta .* log((1-p)./(1 - w(j-1,1:end)));
    ...
    log_asset_trj(j,1:end) = log_asset_trj(j-1,1:end) + mu*dt + K0 + ...
            K1.*vola_trj(j-1,1:end) + K2.*vola_trj(j,1:end) + ...
            sqrt(K3 .* vola_trj(j-1,1:end) + K4 .* vola_trj(j,1:end)) .* [w2(j-1,1:end), -w2(j-1,1:end)];
